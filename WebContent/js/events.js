...
function canvasOnLoad() {
	...
	dojo.event.connect(document, 'onkeydown', function(evt) {
        if(evt.keyCode==evt.KEY_ESCAPE) {		          //ESC_KEY = dojo cometd ESC key breaks FF bug
			setTimeout('handleLogout()', 50);
			dojo.event.browser.stopEvent(evt);
			return false;
		} else if(evt.keyCode==evt.KEY_TAB) {	          //TAB_KEY = tab into next dialog
            var focusDialog = getInFocusDialog();
            if(focusDialog) focusOnDialog(focusDialog);
			dojo.event.browser.stopEvent(evt);
		} else if(evt.keyCode > 48 && evt.keyCode < 58 && 
                  evt.ctrlKey) {                          //Ctrl + 1, 2 = open dialog to 1st, 2nd etc buddy
			try {
				var buddyListContent = document.getElementById('buddyListContent');
				var buddyContainer = buddyListContent.getElementsByTagName('DIV')[evt.keyCode - 49];
				buddyContainer.onclick();
			} catch(e) {}
			dojo.event.browser.stopEvent(evt);
	    } else if(evt.keyCode==A_KEY && evt.ctrlKey) {    //Ctrl + A = change my status to 'Away: ...'
            var customAwayMsg = prompt('Please enter your away message:', '');
            if(customAwayMsg != null) changeUserStatus( USER_STATUS_AWAY, customAwayMsg );
            dojo.event.browser.stopEvent(evt);
        } else if(evt.keyCode==O_KEY && evt.ctrlKey) {    //Ctrl + O = change my status to 'Online'
            changeUserStatus( USER_STATUS_ONLINE );
            if(IS_IE) window.event.keyCode = 505; //needed because IE would still pop the 'Open URL' box
            dojo.event.browser.stopEvent(evt);
        } else if(evt.keyCode==S_KEY && evt.ctrlKey) {    //Ctrl + S = turn sound ON or OFF
            ...
            dojo.event.browser.stopEvent(evt);
        } else if(evt.keyCode==B_KEY && evt.ctrlKey) {    //Ctrl + B = turn blinker ON or OFF
            ...
            dojo.event.browser.stopEvent(evt);
        } else if(evt.keyCode==W_KEY && evt.ctrlKey) {    //Ctrl + W = turn wallpaper ON or OFF
    ...
    if(IS_IE) {
        ...
        contentFrame.contentDocument.addEventListener('focus', canvasOnFocus, false);
    } else {
...
function canvasOnUnLoad() {
    ...
    if(IS_IE && SESSION.valid && (window.screenTop <= 10000)) {	//ie and browser open
	  alert('Copyright 2006-2008 Z, Inc. All rights reserved.\n\n' +
			...
			'Thanks!');
	  handleLogout();
    } else if(SESSION.valid) {									//not ie or browser closed
	  handleLogout();
    }
...
function pushHandler(msg) {
    if(msg.data) {
        ...
        switch(msg.data.eventType) {
            case EVENT_TYPE_GET_USERS:                 handleGetUsersEvent(msg.data);            break;
            case EVENT_TYPE_GET_OFFLINE_MESSAGES:      handleGetOfflineMessagesEvent(msg.data);  break;
            case EVENT_TYPE_INCOMING_MESSAGE:          handleIncomingMessageEvent(msg.data);     break;
            case EVENT_TYPE_USER_CHANGE_STATUS:        handleUserChangeStatusEvent(msg.data);    break;
            case EVENT_TYPE_USER_CHANGE_BUDDY_ICON:    handleUserChangeBuddyIconEvent(msg.data); break;
            case EVENT_TYPE_USER_TYPING:               handleUserTypingEvent(msg.data);          break;
            case EVENT_TYPE_SYSTEM_MESSAGE:            handleSystemMessageEvent(msg.data);       break;
            case EVENT_TYPE_SPECIAL_EFFECT:            handleSpecialEffectEvent(msg.data);       break;
            default: alert('pushHandler: Unknown eventType: ' + msg.data.eventType); break;
        }
    } else {
...
function handleGetUsersEvent(msg) {
	for(var i=0; i<msg.users.length; i++) {
		if( msg.users[i].userName != USERNAME ) {               //don't display current user on buddy list
            paintUserOnBuddyList(msg.users[i]);
		} else if( msg.users[i].userName == USERNAME ) {        //paint current user's buddy icon on the buddy list (buddy icons are stored on the server side only!!)
...
function handleIncomingMessageEvent(msg) {
	...
	if(!dialog) {
        ...
        if(buddyOnBuddyList) buddyIconSrc = buddyOnBuddyList.getElementsByTagName('IMG')[0].src;
        else if(msg.sourceBuddyIcon != BUDDY_ICON_UNSET) buddyIconSrc = msg.sourceBuddyIcon;
...
function handleUserChangeStatusEvent(msg) {
	...
	if(container) {		//user already painted on buddy list --> simply change the status or remove (if status is OFFLINE)
        ...
        if(msg.status == USER_STATUS_OFFLINE) {
            ...
            if(dojo.widget.manager.getWidgetById('showOfflineCheckbox').checked) paintUserOnBuddyList(msg);
            ...
            if(USERS[msg.userName].status == USER_STATUS_AWAY) {
                ...
                container.getElementsByTagName('IMG')[1].src = IMG_LED_STATUS_ONLINE_SRC;
		...
		} else {              //USER_STATUS_AWAY
			container.getElementsByTagName('IMG')[1].src = IMG_LED_STATUS_AWAY_SRC;
			container.getElementsByTagName('SPAN')[0].innerHTML = getUserStatusAsText(msg.status);
            if(msg.customAwayMsg != '') {
                ...
                if(msg.customAwayMsg.length > 12) container.getElementsByTagName('SPAN')[0].innerHTML += ': ' + msg.customAwayMsg.substr(0, 12) + '...';
                else container.getElementsByTagName('SPAN')[0].innerHTML += ': ' + msg.customAwayMsg;
            }
            //do NOT play sound if we're going to AUTO AWAY
		    if(msg.customAwayMsg != AUTO_AWAY_CUSTOM_MESSAGE) playSound(SOUND_BUDDY_LOGOUT);
		}
		paintSystemMessageOnDialogAboutUserChangeStatus(msg, false);
	} else {			//user not yet painted on buddy list --> let's paint it now!
...
function handleSpecialEffectEvent(msg) {
    ...
    switch(msg.effectType) {
        case SPECIAL_EFFECT_TYPE_SOUND:
            paintSystemMessageOnDialogAboutIncomingSpecialEffect(msg.source, msg.effectType, msg.effectId, false);
...
function sendMessage(chatPartner, msg) {
	...
	var mb = { eventType:	EVENT_TYPE_INCOMING_MESSAGE,
		       source:		USERNAME,
			   target:		chatPartner,
			   ...
			   hour:		hour,
			   minute:		minute,
			   style:       { color:           dialog.ta.style.color,
...
function sendUserStatus(status, customAwayMsg) {
	//publish message via comet
    var ub = { eventType:	    EVENT_TYPE_USER_CHANGE_STATUS,
               userName:	    USERNAME,
               status:		    status,
...
function changeUserStatus(newStatusInt, customAwayMsg) {
    ...
    if(customAwayMsg == null) {
        ...
        if( customAwayMsg != AUTO_AWAY_CUSTOM_MESSAGE ) document.getElementById('myCustomAwayMsg').value = customAwayMsg;
    ...
    if(newStatusInt == USER_STATUS_AWAY && customAwayMsg != '') {
        ...
        if(customAwayMsg.length > 10) newStatusText += ':<br>' + customAwayMsg.substr(0, 10) + '...';
        else newStatusText += ':<br>' + customAwayMsg;
    }

    if(newStatusInt != USERS[USERNAME].status ||
...
function sendChatStatus(chatPartner, status) {
	//publish message via comet
	var mb = { eventType:	EVENT_TYPE_USER_TYPING,
		       source:		USERNAME,
			   status:		status };
	cometd.publish(chatPartner, mb);
}
...
function sendBuddyIcon(newBuddyIconSrc) {
	...
	if(!newBuddyIconSrc) newBuddyIconSrc = rewriteURL(top.cairo.customBuddyIcon.file);

	//publish message via comet
	var ub = { eventType:	EVENT_TYPE_USER_CHANGE_BUDDY_ICON,
			   userName:	USERNAME,
...
function sendSpecialEffect(chatPartner, effectType, effectId) {
	//publish message via comet
	var mb = { eventType:	EVENT_TYPE_SPECIAL_EFFECT,
		       source:		USERNAME,
...
function handleWallpaperPickerEvent() {
	...
	if(customWallpaperForm) customWallpaperForm.parentNode.removeChild(customWallpaperForm);
...
function handleMyBuddyIconChangeEvent() {
	...
	if(customBuddyIconForm) customBuddyIconForm.parentNode.removeChild(customBuddyIconForm);
...
function handleCustomPickerUploadEvent(fileInput, targetDir, storeImg) {
    ...
    if( isValidFileName(fileName, IMAGE_FILE_EXTENSIONS, true) ) {
        ...
        var targetFileName = targetDir +								//create targetFileName (USERNAME_1.gif)
                             USERNAME + '_' + storeImg.counter +
                             '.' + extension;
...
function handleFileUploadEvent(fileInput) {
    ...
    if( isValidFileName(fileName, FILE_EXTENSIONS, true) ) {
        ...
        top.cairo.fileUpload[fileInput.form.currentTime.value].file = targetFileName;
        fileInput.form.targetFileName.value = targetFileName;
        fileInput.form.submit();										//submit the form
    } else {
      //remove the just uploaded file's form
      var sendFileForm = document.getElementById('sendFileForm_'+fileInput.form.currentTime.value);
...
function handleClickFilterEvent() {
	if(window.hideObject.length > 0) {
		...
		for(var i=0; i<toHide.obj.length; i++) toHide.obj[i].style.visibility = 'hidden';
		if(toHide.cssObj) {
            var temp = toHide.cssObj[1];  // need to save it out, because toHide becomes null!
            toHide.cssObj[0].className = toHide.cssObj[1];
			toHide.cssObj[0].onmouseout = function (e) { clearClassName(e, this, temp); };
		}
		toHide = null;
		...
		var cf = document.getElementById('clickFilter' + (window.hideObject.length + 1));
        if(cf) {
          cf.style.width = '0px';
          cf.style.height = '0px';
        }
...
function startAutoAway() {
    if(USERS[USERNAME].status == USER_STATUS_ONLINE) top.cairo.autoAway.timeOut = setTimeout('putUserToAutoAway()', top.cairo.autoAway.maxIdleTime * 60 * 1000);
}
...
function stopAutoAway() {
    if( top.cairo.autoAway.timeOut != null ) clearTimeout(top.cairo.autoAway.timeOut);
