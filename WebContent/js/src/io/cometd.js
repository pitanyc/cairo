	...
	this.openTunnelWith = function(content, url){
		dojo.io.bind({
			...
			error: function(type, errObj){
                //dojo.debug("tunnel opening failed: " + errObj);
                //cometd.deliver(data);
				//this.connected = false;
				//this.tunnelCollapse();
                ...
                setTimeout(function() {
                    cairoTunnelFix(content, url, 1);
                }, 500);

                //dojo.lang.setTimeout(this, openTunnelWith, 1000, content, url);
...
function cairoTunnelFix(content, url, counter) {

    if(counter == 2) {
        top.document.title = 'Cairo DIED';
        alert('Your connection has collapsed and needs to be reset.\n\nPlease login again.');
        window.onbeforeunload = null;
        //handleLogout();
    } else {
        top.document.title = 'Cairo ISSUE';
        alert('Connection problem, attempting to reconnect (#' + counter + ')');
        ...
        dojo.io.bind({
            ...
            error: function(type, errObj){
                ...
                document.getElementById('waitIndicator').style.visibility = 'visible';
                counter++;
                dojo.lang.setTimeout(cairoTunnelFix, 500, content, url, counter);
             }
        });
    }
