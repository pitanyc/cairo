...
function Dialog(chatPartner, chatPartnerBuddyIconSrc) {
  ...
  this.chatStatus = { lastChatStatus: CHAT_STATUS_NOT_TYPING,
	                  timeOut: null };						//chat status feature
  this.nudge = { enabled: false,
                 step: 5,
                 speed: 100,
                 timeOut: null };                           //nudge feature
  ...
  this.translate = { enabled: false,                                         //shortcut to the translate items on this dialog
                        from: '',
                          to: 'en' };
...
/*  ====================================================================================================
  ...
  function createHeader(chatPartner) {
    ...
    var maxImg = document.createElement('A');
      ...
      maxImg.onmouseover = function (e) {							//onmouseover style effects
        if(this.className != 'topbarMaxhover') this.className = 'topbarMaxhover';
      };
      maxImg.onmouseout = function (e) {							//onmouseout style effects
    ...
    var minImg = document.createElement('A');
      ...
      minImg.onmouseover = function (e) {							//onmouseover style effects
        if(this.className != 'topbarMinhover') this.className = 'topbarMinhover';
      };
      minImg.onmouseout = function (e) {							//onmouseout style effects
    ...
    var buddyUserStatusImg = document.createElement('IMG');
      ...
      if( USERS[chatPartner] ) {    //need this, because offline users have nothing in USERS[chatPartner]
        if( USERS[chatPartner].status == USER_STATUS_ONLINE ) buddyUserStatusImg.src = IMG_LED_STATUS_ONLINE_SRC;
          ...
          if(firstColon > -1) {
            ...
            var customAwayMsg = getBuddy(chatPartner).title.substr(firstColon+2);
            if(customAwayMsg.length > 20) customAwayMsg = customAwayMsg.substr(0, 20) + '...';
    ...
    var buddyName = document.createElement('DIV');
	  ...
	  if(chatPartner.length > 15) buddyName.appendChild(document.createTextNode(chatPartner.substr(0, 15) + '...'));
    ...
    var header = document.createElement('DIV');
      ...
      header.ondblclick = function(e) {
        ...
        var resizeHandle = getElementsByClassName(container, 'img', 'resizeHandle')[0];
        if(that.maximized) {    //restore down already maximized object
          ...
          var maximizeImg = getElementsByClassName(container, 'a', 'topbarTiles|topbarTileshover')[0];
          ...
          //show resize handle
          if(resizeHandle) {
            var btmbarRight = getElementsByClassName(container, 'span', 'btmbarRight')[0];
          ...
          // update members
          that.maximized = { left:					container.offsetLeft,
          ...
          // hide resize handle
          if(resizeHandle) {                                        //hide resize handle
            resizeHandle.style.display = 'none';
            var btmbarResize = getElementsByClassName(container, 'span', 'btmbarResize')[0];
          ...
          // swap maximize / tiles images
          var maximizeImg = getElementsByClassName(container, 'a', 'topbarMax|topbarMaxhover')[0];
  ...
  function createChatHistoryDiv() {
    ...
    var historyDiv = document.createElement('DIV');
      ...
      historyDiv.onscroll = function(e) {
        ...
        if(that.autoScroll.scrollerLength == 0) that.autoScroll.scrollerLength = newScrollerLength;
        else {
          if((newScrollerLength != that.autoScroll.scrollerLength) && that.autoScroll.enabled) {
          ...
          } else if((newScrollerLength == that.autoScroll.scrollerLength) && !that.autoScroll.enabled) {
  ...
  function createChatControlsAndTextarea(chatPartner) {
    ...
    var chatStatusSpan = document.createElement('SPAN');
      ...
      chatStatusSpan.innerHTML = getChatStatusAsText(chatPartner, CHAT_STATUS_NOT_TYPING);
    ...
    var resizeHandle = document.createElement('IMG');
      ...
      resizeHandle.onmousedown = function(e) {
        e = e || event;
        window.resizeObject = { grabX:          e.clientX,
                                grabY:          e.clientY - 20,
                                dialog:         that,
  ...
  function createFooter() {
    ...
    var btmbarResize = document.createElement('SPAN');
      ...
      btmbarResize.onmousedown = function(e) {
        var resizeHandle = getElementsByClassName(that.DOM, 'img', 'resizeHandle')[0];
  ...
  function createContainer(header, historyDiv, chatControls, footer) {
	...
	container.onkeydown = function(e) {
      e = e || event;
      if(e.keyCode==ESC_KEY) {						//ESC_KEY = close this dialog
		removeDialog();
        dojo.event.browser.stopEvent(e);
		return false;
	  } else if(e.keyCode==TAB_KEY) {				//TAB_KEY = tab into next dialog
	  ...
	  } else if(e.keyCode==PAGEUP_KEY) {			//PAGEUP_KEY = scroll chat history up
        that.screen.scrollTop -= 30;
	  } else if(e.keyCode==PAGEDOWN_KEY) {			//PAGEDOWN_KEY = scroll chat history down
        that.screen.scrollTop += 30;
	  } else if(e.keyCode==X_KEY && e.ctrlKey) {	//Ctrl + X = maximize this dialog
        handleClickFilterEvent();
        var maximizeImg = getElementsByClassName(that.DOM, 'a', 'topbarMax|topbarMaxhover|topbarTiles|topbarTileshover')[0];
        maximizeImg.onclick();
        dojo.event.browser.stopEvent(e);
      } else if(e.keyCode==UP_KEY && e.ctrlKey && !that.maximized) {	//Ctrl + Up = move this dialog up
		handleClickFilterEvent();
        var px = new RegExp('px');
		this.style.top = this.style.top.replace(px, '') - DIALOG_MIN_Y_MOVE + 'px';
	  } else if(e.keyCode==DOWN_KEY && e.ctrlKey && !that.maximized) {	//Ctrl + Down = move this dialog down
        handleClickFilterEvent();
        var px = new RegExp('px');
		this.style.top = this.style.top.replace(px, '') - (-DIALOG_MIN_Y_MOVE) + 'px';
	  } else if(e.keyCode==LEFT_KEY && e.ctrlKey && !that.maximized) {	//Ctrl + Left = move this dialog to the left
        handleClickFilterEvent();
        var px = new RegExp('px');
		this.style.left = this.style.left.replace(px, '') - DIALOG_MIN_X_MOVE + 'px';
	  } else if(e.keyCode==RIGHT_KEY && e.ctrlKey && !that.maximized) {	//Ctrl + Right = move this dialog to the right
        handleClickFilterEvent();
        var px = new RegExp('px');
		this.style.left = this.style.left.replace(px, '') - (-DIALOG_MIN_X_MOVE) + 'px';
	  }
	...
	container.ondrop = function(e) {
      ...
      top.cairo.fileUpload[currentTime] = { chatPartner:that.chatPartner, file:'' };
      var fileName = e.dataTransfer.mozGetDataAt('text/x-moz-url', 0);
      loadURLIntoContainer('/sendFileForm.jsp?ct='+currentTime+'&fn='+fileName, 'sendFileForm_'+currentTime, false);
      dojo.event.browser.stopEvent(e);
    };
  ...
  function createFontFamilyPicker() {
    ...
    var defaultFontFamily = getCookieProperty(COOKIE_FONTSTYLE, DIALOG_FONT_STYLE_FONT_FAMILY);
	if(defaultFontFamily) {
	  if(defaultFontFamily.length > 12) defaultFontFamily = defaultFontFamily.substr(0, 10) + '...';
    ...
    var fontFamilyPicker = document.createElement('BUTTON');
      ...
      fontFamilyPicker.onclick = function(e) {
        this.onmouseout = function (e) { this.className = 'comboboxFocus'; };

        //get the font family palette
        var fontFamilyPalette = document.getElementById('fontFamilyPalette');
        if(!fontFamilyPalette) {	//font family palette has never been created --> let's paint it now!
          ...
          for(var i=0; i<FONTS.length; i++) {
            if(FONTS[i].detect) {		//show only detected font types to the user!
              //the next row
              var nextRow = fontFamilyPalette.insertRow(-1);
              nextRow.className = '';												//by default each entry is NOT in focus
              nextRow.onmouseover = function (e) { this.className = 'focus'; };		//onmouseover effects
              nextRow.onmouseout = function (e) { this.className = ''; };			//onmouseout effects
              nextRow.onclick = function(e) {
                ...
                if(fontFamily.length > 12) fontFamilySpan.innerHTML = fontFamily.substr(0, 10) + '...';	//if selected font-family too long, truncate it first...
                ...
                setCookieProperty(COOKIE_FONTSTYLE, DIALOG_FONT_STYLE_FONT_FAMILY, fontFamily);	//save out picked font family
              };
        ...
        //show click filter
        window.hideObject.push( { obj: [ fontFamilyPalette ], cssObj: [ fontFamilyPicker, 'combobox' ] } );
        maximizeClickFilter();
        document.getElementById('clickFilter' + window.hideObject.length).style.zIndex = 21;
        ...
        if(pos.left + fontFamilyPalette.offsetWidth < document.body.offsetWidth) fontFamilyPalette.style.left = pos.left + 'px';	//if there is enough space, paint it towards the RIGHT...
        else fontFamilyPalette.style.left = pos.left + this.offsetWidth - fontFamilyPalette.offsetWidth + 'px';	                    //...otherwise, paint it towards the LEFT
        if(pos.top + this.offsetHeight + fontFamilyPalette.offsetHeight < document.body.offsetHeight) fontFamilyPalette.style.top = pos.top + this.offsetHeight + 'px';	//if there is enough space, paint it towards the BOTTOM...
        else fontFamilyPalette.style.top = pos.top - fontFamilyPalette.offsetHeight + 'px';																			    //...otherwise, paint it towards the TOP
  ...
  function createFontSizePicker() {
    ...
    var defaultFontSize = getCookieProperty(COOKIE_FONTSTYLE, DIALOG_FONT_STYLE_FONT_SIZE);
    if(defaultFontSize) {
	...
	var fontSizePicker = document.createElement('BUTTON');
	  ...
	  fontSizePicker.onclick = function(e) {
		...
		if(!fontSizePalette) {	//font size palette has never been created --> let's paint it now!
		  ...
		  for(var i=0; i<FONTS.length; i++) {
			if(FONTS[i].size != '') {
			  ...
			  nextRow.onclick = function(e) {
			    ...
			    focusDialog.ta.focus();									        //focus ta
			    setCookieProperty(COOKIE_FONTSTYLE, DIALOG_FONT_STYLE_FONT_SIZE, fontSize);	//save out picked font size
			  };
		...
		//show click filter
		window.hideObject.push( { obj: [ fontSizePalette ], cssObj: [ fontSizePicker, 'combobox' ] } );
		maximizeClickFilter();
        document.getElementById('clickFilter' + window.hideObject.length).style.zIndex = 21;
		...
		if(pos.left + fontSizePalette.offsetWidth < document.body.offsetWidth) fontSizePalette.style.left = pos.left + 'px';	//if there is enough space, paint it towards the RIGHT...
		else fontSizePalette.style.left = pos.left + this.offsetWidth - fontSizePalette.offsetWidth + 'px';			            //...otherwise, paint it towards the LEFT
        if(pos.top + this.offsetHeight + fontSizePalette.offsetHeight < document.body.offsetHeight) fontSizePalette.style.top = pos.top + this.offsetHeight + 'px';	//if there is enough space, paint it towards the BOTTOM...
        else fontSizePalette.style.top = pos.top - fontSizePalette.offsetHeight + 'px';																											//...otherwise, paint it towards the TOP
  ...
  function createColorPicker() {
    ...
    var colorPicker = document.createElement('IMG');
      ...
      colorPicker.onmouseover = function (e) {
        if(this.className != 'formatleftHover') this.className = 'formatleftHover';
      };
      colorPicker.onmouseout = function (e) {
        if(this.className != 'formatleft') this.className = 'formatleft';
      };
      colorPicker.onclick = function(e) {
        this.onmouseout = function (e) { this.className = 'formatleftHover'; };

        //get the color palette
        var colorPalette = document.getElementById('colorPalette');
        if(!colorPalette) {	//color palette has never been created --> let's paint it now!
          ...
          var widget = dojo.widget.createWidget('dojo:ColorPalette', {palette:'7x10'}, colorPaletteChildDiv);
        ...
        //show click filter
        window.hideObject.push( { obj: [ colorPalette ], cssObj: [ colorPicker, 'formatleft' ] } );
        maximizeClickFilter();
        ...
        if(pos.left + colorPalette.offsetWidth < document.body.offsetWidth) colorPalette.style.left = pos.left + 'px';	//if there is enough space, paint it towards the RIGHT...
        else colorPalette.style.left = pos.left + this.offsetWidth - colorPalette.offsetWidth + 'px';                   //...otherwise, paint it towards the LEFT
        if(pos.top - colorPalette.offsetHeight > 0) colorPalette.style.top = pos.top - colorPalette.offsetHeight + 'px';//paint it towards the TOP...
  ...
  function createLanguagePicker(cssId) {
    ...
    var langPicker = document.createElement('BUTTON');
      ...
      langPicker.onclick = function(e) {
        ...
        if(!languagePalette) {	//language palette has never been created --> let's paint it now!
          ...
          for(var i = (cssId == 'languagePaletteFrom') ? 0 : 1; i<LANGUAGES.length; i++) {
            //the next row
            var nextRow = languagePalette.insertRow(-1);
              ...
              nextRow.onclick = function(e) {
                ...
                if(selectedLanguage.length > 12) selectedLanguage = selectedLanguage.substr(0, 12) + '...';	    //if selected language too long, truncate it first
                if(cssId == 'languagePaletteFrom') {
                  focusDialog.translate.from = this.firstChild.code;
                  document.getElementById('translatePalette').getElementsByTagName('SPAN')[1].innerHTML = selectedLanguage;
                } else {
                  focusDialog.translate.to = this.firstChild.code;
                  document.getElementById('translatePalette').getElementsByTagName('SPAN')[2].innerHTML = selectedLanguage;
                }
        ...
        //show click filter
        window.hideObject.push( { obj: [ languagePalette ], cssObj: [ langPicker, 'combobox' ] } );
        maximizeClickFilter();
        document.getElementById('clickFilter' + window.hideObject.length).style.zIndex = 21;
        ...
        //show font family palette
        if(pos.left + languagePalette.offsetWidth < document.body.offsetWidth) languagePalette.style.left = pos.left + 'px';	//if there is enough space, paint it towards the RIGHT...
        else languagePalette.style.left = pos.left + this.offsetWidth - languagePalette.offsetWidth + 'px';	                    //...otherwise, paint it towards the LEFT
        if(pos.top + this.offsetHeight + languagePalette.offsetHeight < document.body.offsetHeight) languagePalette.style.top = pos.top + this.offsetHeight + 'px';	//if there is enough space, paint it towards the BOTTOM...
        else languagePalette.style.top = pos.top - languagePalette.offsetHeight + 'px';																			    //...otherwise, paint it towards the TOP
  ...
  function updateTranslatePicker(dialog) {
    var translatePicker = getElementsByClassName(dialog.DOM, 'img', 'formatleft|formatleftHover')[2];
    if(dialog.translate.enabled) {
      var fromLanguage = dialog.translate.from;
      var toLanguage = dialog.translate.to;
      for(var i=0; i<LANGUAGES.length; i++) {
        if(LANGUAGES[i].code == dialog.translate.from) fromLanguage = LANGUAGES[i].name;
        if(LANGUAGES[i].code == dialog.translate.to) toLanguage = LANGUAGES[i].name;
      }
  ...
  function createTranslatePicker() {
    ...
    var translatePicker = document.createElement('IMG');
      ...
      translatePicker.onclick = function(e) {
        ...
        if(translatePalette) {  //remove previously painted translate palette (possibly for another dialog)
          ...
          //checkbox
          var tranCheckbox = dojo.widget.createWidget('Checkbox', {id: 'translateCheckbox', widgetId: 'translateCheckbox', name: 'translateCheckbox'});
        ...
        //show click filter
        window.hideObject.push( { obj: [ translatePalette ], cssObj: [ translatePicker, 'formatleft' ] } );
        maximizeClickFilter();
        ...
        if(pos.left + translatePalette.offsetWidth < document.body.offsetWidth) translatePalette.style.left = pos.left + 'px';      //if there is enough space, paint it towards the RIGHT...
        else translatePalette.style.left = pos.left + this.offsetWidth - translatePalette.offsetWidth + 'px';                       //...otherwise, paint it towards the LEFT
        if(pos.top - translatePalette.offsetHeight > 0) translatePalette.style.top = pos.top - translatePalette.offsetHeight + 'px';//paint it towards the TOP...
  ...
  function createFontStylePicker() {
    ...
    var fontStylePicker = document.createElement('IMG');
      ...
      fontStylePicker.onclick = function(e) {
        ...
        if(!fontStylePalette) {	//fontStylePalette has never been created --> let's paint it now!
          ...
          var bElement = document.createElement('IMG');
            ...
            bElement.onmouseover = function (e) {
              if(!this.className.match('formatleftHover ')) this.className = 'formatleftHover ' + (this.checked ? 'bElementOn' : 'bElementOff');
            };
            bElement.onmouseout = function (e) {
              if(!this.className.match('formatleft ')) this.className = 'formatleft ' + (this.checked ? 'bElementOn' : 'bElementOff');
            };
          ...
          var iElement = document.createElement('IMG');
            ...
            iElement.onmouseover = function (e) {
              if(!this.className.match('formatleftHover ')) this.className = 'formatleftHover ' + (this.checked ? 'iElementOn' : 'iElementOff');
            };
            iElement.onmouseout = function (e) {
              if(!this.className.match('formatleft ')) this.className = 'formatleft ' + (this.checked ? 'iElementOn' : 'iElementOff');
            };
          ...
          var uElement = document.createElement('IMG');
            ...
            uElement.onmouseover = function (e) {
              if(!this.className.match('formatleftHover ')) this.className = 'formatleftHover ' + (this.checked ? 'uElementOn' : 'uElementOff');
            };
            uElement.onmouseout = function (e) {
              if(!this.className.match('formatleft ')) this.className = 'formatleft ' + (this.checked ? 'uElementOn' : 'uElementOff');
            };
            uElement.onclick = function(e) {
              var focusDialog = getInFocusDialog();
              if(!uElement.checked) {
                focusDialog.ta.style.textDecoration = 'underline';
                setCookieProperty(COOKIE_FONTSTYLE, DIALOG_FONT_STYLE_UNDERLINE, '1');
                uElement.checked = true;
                uElement.className = 'formatleftHover uElementOn';
              } else {
                focusDialog.ta.style.textDecoration = 'none';
                setCookieProperty(COOKIE_FONTSTYLE, DIALOG_FONT_STYLE_UNDERLINE, '0');
                uElement.checked = false;
        ...
        //show click filter
        window.hideObject.push( { obj: [ fontStylePalette ], cssObj: [ fontStylePicker, 'formatleft' ] } );
        maximizeClickFilter();
        ...
        //show the palette
        if(pos.left + fontStylePalette.offsetWidth < document.body.offsetWidth) fontStylePalette.style.left = pos.left + 'px';    //if there is enough space, paint it towards the RIGHT...
        else fontStylePalette.style.left = pos.left + this.offsetWidth - fontStylePalette.offsetWidth + 'px';                     //...otherwise, paint it towards the LEFT
        if(pos.top - fontStylePalette.offsetHeight > 0) fontStylePalette.style.top = pos.top - fontStylePalette.offsetHeight + 'px';  //if there is enough space, paint it towards the TOP...
  ...
  function createSoundPicker() {
    ...
    var soundsPicker = document.createElement('IMG');
      ...
      soundsPicker.onmouseover = function (e) {
        if(this.className != 'formatrightHover') this.className = 'formatrightHover';
      };
      ...
      soundsPicker.onclick = function(e) {
        this.onmouseout = function (e) { this.className = 'formatrightHover'; };

        //get the sounds palette
        var soundsPalette = document.getElementById('soundsPalette');
        if(!soundsPalette) {	//sounds palette has never been created --> let's paint it now!
          ...
          for(var i=0; i<SOUNDS.length; i++) {
            ...
            nextCell.onclick = function(e) {
              ...
              var focusDialog = getInFocusDialog();                                     //get the in focus dialog
              sendSpecialEffect(focusDialog.chatPartner,
                                SPECIAL_EFFECT_TYPE_SOUND,
                                this.effectId);                                         //send over the special effect to the chat partner
              paintSystemMessageOnDialogAboutIncomingSpecialEffect(
                  focusDialog.chatPartner,
                  SPECIAL_EFFECT_TYPE_SOUND, 
                  this.effectId,
                  true);//paint System message on self dialog
        ...
        //show click filter
        window.hideObject.push( { obj: [ soundsPalette ], cssObj: [ soundsPicker, 'formatright' ] } );
        maximizeClickFilter();
        ...
        if(pos.left - soundsPalette.offsetWidth > 0) soundsPalette.style.left = pos.left + this.offsetWidth - soundsPalette.offsetWidth + 'px'; //if there is enough space, paint it towards the LEFT
        else soundsPalette.style.left = pos.left + 'px';                                                                                        //...otherwise, paint it towards the RIGHT
        if(pos.top - soundsPalette.offsetHeight > 0) soundsPalette.style.top = pos.top - soundsPalette.offsetHeight + 'px'; //if there is enough space, paint it towards the TOP
  ...
  function createSmileyPicker() {
    ...
    var smileyPicker = document.createElement('IMG');
      ...
      smileyPicker.onclick = function(e) {
        ...
        if(!smileyPalette) {	//smiley palette has never been created --> let's paint it now!
          ...
          for(var i=0; i<SMILEYS.length; i++) {
            ...
            nextCell.keyCode = nextCell.keyCode.substr(0, nextCell.keyCode.length - 3);	//cut '/i' from the end
            nextCell.keyCode = nextCell.keyCode.replace(/\\/g, '');						//remove all '\' chars
            if(nextCell.keyCode.search(/\[/g) > -1) {                                   //select first option from [:,;] kind of group
              nextCell.keyCode = nextCell.keyCode.substr(nextCell.keyCode.search(/\[/g) + 1);
              nextCell.keyCode = nextCell.keyCode.substr(0, nextCell.keyCode.search(/,/g)) + nextCell.keyCode.substr(nextCell.keyCode.search(/\]/g) + 1);
            }
            nextCell.onmouseover = function (e) { this.className = 'focus'; };			//onmouseover effects
            nextCell.onmouseout = function (e) { this.className = ''; };				//onmouseout effects
            nextCell.onclick = function(e) {
        ...
        //show click filter
        window.hideObject.push( { obj: [ smileyPalette ], cssObj: [ smileyPicker, 'formatright' ] } );
        maximizeClickFilter();
        ...
        if(pos.left - smileyPalette.offsetWidth > 0) smileyPalette.style.left = pos.left + this.offsetWidth - smileyPalette.offsetWidth + 'px';	//if there is enough space, paint it towards the LEFT
        else smileyPalette.style.left = pos.left + 'px';	                                                                                    //...otherwise, paint it towards the RIGHT
        if(pos.top - smileyPalette.offsetHeight > 0) smileyPalette.style.top = pos.top - smileyPalette.offsetHeight + 'px';						//if there is enough space, paint it towards the TOP
  ...
  function setDialogDefaults() {
	...
	// init font color
	var defaultFontColor = getCookieProperty(COOKIE_FONTSTYLE, DIALOG_FONT_STYLE_COLOR);
	if(defaultFontColor) {
    ...
    if(that.translate.enabled) {
      getElementsByClassName(that.DOM, 'img', 'formatleft')[2].src = IMG_TRANSLATE_PICKER_ON_SRC;
    }
...
/*  ====================================================================================================
  ...
  this.maximizeDialog = function() {
    ...
    if(iwinActive == null) iwinActive = getElementsByClassName(that.DOM, 'div',  'iwinInactive')[0];    //needed when maximizing a hidden (otherwise minimized visually) dialog
    ...
    var btmmenuBottom   = getElementsByClassName(that.DOM, 'span', 'btmmenuBottom')[0];
    ...
    if(IS_IE) that.ta.style.width = totalWidth - 20 + 'px'; // 20 = 300 - 280 = IE textarea width
	else if(IS_CHROME || IS_SAFARI) that.ta.style.width = totalWidth - 22 + 'px'; // Chrome textarea width
  ...
  this.restoreDownDialog = function() {
    ...
    var totalHeight = Math.max(DIALOG_MIN_HEIGHT, that.maximized.containerHeight);  //total height of restored object
  ...
  function attemptSendMessage(e) {
	if( (e.keyCode==ENTER_KEY) && (!e.shiftKey) ) {
	  ...
	  var replacedMsg = msg;
	  if(msg.indexOf('%0D%0A') > -1) {		//Windows encodes returns as \r\n hex
		var endOfLine = new RegExp('%0D%0A','g');
        replacedMsg=replacedMsg.replace(endOfLine, '');
		var lastIndex = msg.lastIndexOf('%0D%0A');
		if( (lastIndex + '%0D%0A'.length) == msg.length) msg=msg.substring(0, lastIndex);
		else msg=msg.substring(0, lastIndex) + msg.substring(lastIndex + '%0D%0A'.length);
	  } else if(msg.indexOf('%0A') > -1) {	//Unix encodes returns as \n hex
		var endOfLine = new RegExp('%0A','g');
		replacedMsg=replacedMsg.replace(endOfLine, '');
		var lastIndex = msg.lastIndexOf('%0A');
		if( (lastIndex + '%0A'.length) == msg.length) msg=msg.substring(0, lastIndex);
		else msg=msg.substring(0, lastIndex) + msg.substring(lastIndex + '%0A'.length);
	  } else if(msg.indexOf('%0D') > -1) {	//Macintosh encodes returns as \r hex
		var endOfLine = new RegExp('%0D','g');
		replacedMsg=replacedMsg.replace(endOfLine, '');
		var lastIndex = msg.lastIndexOf('%0D');
		if( (lastIndex + '%0D'.length) == msg.length) msg=msg.substring(0, lastIndex);
		else msg=msg.substring(0, lastIndex) + msg.substring(lastIndex + '%0D'.length);
	  }
	  msg=unescape(msg);
  ...
  function attemptShowSendHistory(e) {
	if( that.sendHistory.arr.length > 0 ) {
	  ...
	  if(e.keyCode==UP_KEY && e.altKey && !e.ctrlKey) {
		...
		if( that.sendHistory.ptr < 0 ) that.sendHistory.ptr = that.sendHistory.arr.length - 1;
		if( that.sendHistory.ptr >= 0 ) that.ta.value = that.sendHistory.arr[that.sendHistory.ptr];
	  } else if(e.keyCode==DOWN_KEY  && e.altKey && !e.ctrlKey) {
		that.sendHistory.ptr++;
		if( that.sendHistory.ptr==that.sendHistory.arr.length ) that.sendHistory.ptr = 0;
		if( that.sendHistory.ptr >= 0 ) that.ta.value = that.sendHistory.arr[that.sendHistory.ptr];
	  }
	}
  }
  ...
  function attemptSendChatStatus(e, chatStatus) {
	...
	if((e.keyCode==ENTER_KEY && chatStatus!=CHAT_STATUS_NOT_TYPING) ||
	    e.keyCode==ESC_KEY      ||
	    e.keyCode==TAB_KEY      || 
	...
	if(chatStatus == CHAT_STATUS_TYPING) {
	  clearTimeout(that.chatStatus.timeOut);
	  that.chatStatus.timeOut = setTimeout('informChatPartnerAboutTypingStopped("' + that.id + '")', 5000);
	}
  }
...
function changeDialogOpacity(dialogId, opacity, isDown) {
  ...
  // structural elements
  var topbarActive  = getElementsByClassName(dialog.DOM, 'div', 'topbarActive|topbarInactive')[0];
  var topmenuActive = getElementsByClassName(dialog.DOM, 'div', 'topmenuActive|topmenuInactive')[0];
  var topmenuLeft   = getElementsByClassName(topmenuActive, 'span', 'topmenuLeft')[0];
  var topmenuImg    = getElementsByClassName(topmenuActive, 'span', 'topmenuImg')[0];
  var topmenuMid    = getElementsByClassName(topmenuActive, 'span', 'topmenuMid')[0];
  var topmenuRight  = getElementsByClassName(topmenuActive, 'span', 'topmenuRight')[0];
  var topmenuSpc    = getElementsByClassName(topmenuActive, 'span', 'topmenuSpc')[0];
  var iwinActive    = getElementsByClassName(dialog.DOM, 'div', 'iwinActive|iwinInactive')[0];
  var btmmenuActive = getElementsByClassName(dialog.DOM, 'div', 'btmmenuActive|btmmenuInactive')[0];
  var btmbarActive  = getElementsByClassName(dialog.DOM, 'div', 'btmbarActive|btmbarInactive')[0];
  ...
  if(dialog.nudge.enabled) dialog.nudge.timeOut = setTimeout('changeDialogOpacity("'+dialogId+'", '+opacity+', '+isDown+')', dialog.nudge.speed);
}
    ...
    if(DIALOGS[i]) {
	  for(var j in DIALOGS) {
	    if( DIALOGS[j] && 
		   ...
		   (DIALOGS[i].DOM.style.left.replace(px, '') == DIALOGS[j].DOM.style.left.replace(px, '')) &&
		   (DIALOGS[i].DOM.style.top.replace(px, '') == DIALOGS[j].DOM.style.top.replace(px, ''))) {
...
function scrollDownScreen(dialogId) {
  ...
  var newScrollerLength = screen.scrollTop > 0 ? screen.scrollHeight - screen.scrollTop : 0;
  DIALOGS[dialogId].autoScroll = { enabled: true, scrollerLength: newScrollerLength };
...
function minimizeDialog(dialog) {
    ...
    if(textValueLong.length > 10) textValueShort = textValueLong.substr(0, 10) + '...';
    ...
    if(dialog.DOM) {
      ...
      dialog.focus = 0;
      for(var dialogId in DIALOGS) {
        if(DIALOGS[dialogId] && DIALOGS[dialogId].focus > 0) DIALOGS[dialogId].focus--;
      }
    } else {
...
function positionDialog(container) {
  //local consts
  var marginLeft = Math.round((document.body.offsetWidth  - container.offsetWidth)  / 6);	//the X coord of the middle of the header on the first spot
  var marginTop = Math.round((document.body.offsetHeight  - container.offsetHeight) / 4);	//the Y coord of the middle of the header on the first spot
    ...
    for(var j=0; j<retryMax; j++) {
	  ...
	  for(var dialogId in DIALOGS) {
		if(DIALOGS[dialogId] && (dialogId != container.id)) {
		  if((DIALOGS[dialogId].DOM.style.left.replace(px, '') == pair.left) && 
			 (DIALOGS[dialogId].DOM.style.top.replace(px, '') == pair.top)) {	//means that yes, another dialog already occupies pair!
			pair.ok = false;
			break;
		  }
		}
...
function focusOnDialog(theDialog) {
  ...
  if(window.focusObject != theDialog) {	//need to focus on theDialog
    ...
    if(theDialog.DOM) {
      ...
      for(var dialogId in DIALOGS) {    //push back current focus indices in DIALOGS HashMap
        //dojo.debug( dialogId + ' focus: ' + DIALOGS[dialogId].focus );
        if(DIALOGS[dialogId]           &&
           DIALOGS[dialogId].focus > 0 &&
           ((oldFocus == null) || (oldFocus == 0) || (DIALOGS[dialogId].focus < oldFocus))) DIALOGS[dialogId].focus++;
      }
      theDialog.focus = 1;
  ...
  } else {  //dialog already in focus --> just focus the textarea
...
function resizeOnMouseMove(e) {
  if(window.resizeObject) {
    ...
    var totalWidth = Math.max(DIALOG_MIN_WIDTH, window.resizeObject.originalWidth + e.clientX - window.resizeObject.grabX);
    var totalHeight = Math.max(DIALOG_MIN_HEIGHT, window.resizeObject.originalHeight + e.clientY - window.resizeObject.grabY);
    //dojo.debug('totalWidth: '+totalWidth+' | totalHeight: '+totalHeight);

    // structural elements
    var topbarMid       = getElementsByClassName(dialog.DOM, 'span', 'topbarMid')[0];
    var topmenuMid      = getElementsByClassName(dialog.DOM, 'span', 'topmenuMid')[0];
    var topmenuSpc      = getElementsByClassName(dialog.DOM, 'span', 'topmenuSpc')[0];
    var buddyName       = getElementsByClassName(dialog.DOM, 'div',  'buddyName')[0];
    ...
    var btmmenuTop      = getElementsByClassName(dialog.DOM, 'span', 'btmmenuTop')[0];
    var btmmenuBottom   = getElementsByClassName(dialog.DOM, 'span', 'btmmenuBottom')[0];
	...
	else if(IS_CHROME || IS_SAFARI) dialog.ta.style.width = totalWidth - 22 + 'px';
